...
`.runtime\knowledge.sqlite3`. Override `--books` on indexing or `--database`
on any reference command as needed. Both default directories are ignored
by Git. The base Cadence controller still needs no third-party runtime package.
...
indexed sources block citation retrieval until indexing runs again; extractor
...
Nothing here learns or certifies PCB design expertise. A reasoning agent must
